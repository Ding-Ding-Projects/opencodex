...
import {
  startSchoolModeSync, afterEach, beforeEach, describe, expect, test } from "bun:test";
import {
...
describe("action functions", () => {
  ...
  test("configureSchoolModeApiBase prefixes every request", async () => {
    ...
    // Recording the base and starting the loop are two calls now, not one.
    // They were fused, which meant importing `App.tsx` — which configures the
    // base at module scope — started a 1.5s interval that no test could clean
    // up: it outlived every teardown and fired into a later file's mocked
    // `fetch`, surfacing there as a probe that file never made. The onboarding
    // wizard's "does not probe" assertion caught it, failing in the suite while
    // passing alone.
