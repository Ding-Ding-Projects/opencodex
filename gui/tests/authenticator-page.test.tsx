...
/**
 * Every test's cleanup unmounts BEFORE `teardown()` restores the real
 * globals — and does so inside the same `try` that declared `root`, not in a
 * `finally` (a `finally` block is its own lexical scope in JS; a `const`
 * bound inside `try` is not visible there). Skipping the unmount lets
 * `useAuthenticatorCode`'s real `window.setTimeout` rollover timer outlive
 * the test: its cleanup effect never runs, so the timer can fire after
 * `teardown()` restored the previous globals, calling `fetch` from inside a
 * LATER test file's mocked environment. `onboarding-wizard.test.tsx` caught
 * exactly this once, as a stray `/api/host/authenticator/code` call.
 */
