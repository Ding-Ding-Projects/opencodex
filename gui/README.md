...
| **React Doctor** (`bun run doctor`) | Gating React health check pinned to react-doctor 0.9.2. Pre-push runs it only if `gui/` changed; the Windows CI workflow fails on errors, uploads the JSON report after every run, and keeps warnings visible for review |
