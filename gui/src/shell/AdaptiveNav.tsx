...
import { IconDevices, IconMoon, IconPower, IconSun, IconMonitor, IconX } from "../icons";
...
export default function AdaptiveNav(props: AdaptiveNavProps) {
  const {
    ...
    claudeEnabled, claudeTogglePending, onToggleClaude, onConnectRemote,
  } = props;
  ...
  const panel = (
    ...
    >
      ...
      <div className="m3-nav-foot">
        <button type="button" className="m3-nav-item" onClick={onConnectRemote}
          aria-label={t("remote.connectTitle")} title={t("remote.connectTitle")}>
          <span className="m3-nav-pill" aria-hidden="true"><IconDevices /></span>
          {showLabels && <span className="m3-nav-label">{t("remote.connect")}</span>}
        </button>
