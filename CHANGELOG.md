...
- fix(tests): stop `tests/count-lines-attribution.test.ts` from counting and blaming the whole real repository at module import time, where no per-test timeout applied and a full-depth clone was required; it now builds a two-commit fixture repository in `beforeAll`, pins exact counts and agent/person attribution against that known content, and finishes in well under a second instead of 42 to 110 seconds. To make that possible, `countLines()` is now async, takes `{ root, concurrency, gitShow }` options, and fans its per-file `git show` calls out through the shared `mapWithConcurrency` (moved to `scripts/concurrency.ts` and still re-exported from `line-attribution.ts`), and `countLinesWithAttribution()` takes a `{ root }` option so a precomputed count from another repository is attributed in that repository rather than this one; the new `tests/hunt-rel-02.test.ts` proves the fan-out with an injected delayed `gitShow`
...
- fix(codex): replace every naive `lines.findIndex(l => /^\s*\[/.test(l))` TOML root-boundary check (setRootOpenaiBaseUrl, stripInjectedOpenaiBaseUrl, stripExistingModelProvider, stripRootRoutedModel, setRootModelProvider, setRootModelCatalogPath in inject.ts; readRootTomlString in paths.ts; rootTomlString and hasInjectedOpenaiBaseUrl in injected-marker.ts; the private hasInjectedOpenaiBaseUrl in project-config-warnings.ts) with a shared lexical scanner (`firstStructuralTableIndex`, extracted from subagent-defaults.ts's already-tested line scanner) that tracks multiline-string state and bracket/brace depth, so a root-level multi-line array or triple-quoted string whose own continuation line starts with `[` is no longer misread as the document's first table header; that misdetection used to splice `openai_base_url` into the middle of the array or string, producing a config Codex's own TOML parser rejects while `injectCodexConfig` still reported success
- fix(codex): resolve `model_catalog_json` ownership against this install's real, resolved catalog path (`samePath` + `activeDefaultCatalogPath`) instead of matching by filename alone, so a user's own catalog file that merely happens to be named `opencodex-catalog.json` in an unrelated directory is no longer overwritten, stripped on restore, or silently replaced
- fix(server): guarantee `process.exit` still runs when the shutdown drain throws, for `POST /api/stop`, `POST /api/host/exit`, and the dashboard's drain-and-restart; a throwing `drainAndShutdown` used to become an unhandled rejection that `crash-guard.ts` silently swallows, leaving the daemon running forever after the client was already told it was stopping (or, for the restart route, permanently stuck refusing traffic). `/api/stop` and `/api/host/exit` now share a `scheduleDrainAndExit` helper that mirrors the CLI's own proven `try { await drainAndShutdown(...) } finally { process.exit(...) }` pattern; the restart route resets its accept latch and resumes serving instead of exiting when the drain itself fails.
- perf(scripts): parallelize `attributeTrackedLines`'s per-file `git cat-file`/`git blame` calls through the existing `mapWithConcurrency` (bounded at 8, now a named constant) instead of a synchronous spawn one file at a time, and let `countLinesWithAttribution` accept an already-computed `countLines()` result instead of always recomputing it; line-attribution output is unchanged (verified byte-identical before/after) and `tests/count-lines-attribution.test.ts` drops from ~110s to ~67s
- fix(scripts): give `bun run test` (`scripts/test.ts`) a generous overall wall-clock bound (20 minutes, overridable via `OCX_TEST_WALL_CLOCK_MS`) on every platform, not just the Windows `--timeout`/`--max-concurrency` flags, so a wedged local run fails loudly instead of hanging indefinitely; CI is unaffected, since it invokes `bun test` directly rather than through this script
- fix(tests): stop `tests/disposable-host-acceptance-contract.test.ts` from littering the repository root with `ocx-service-acceptance-*` directories on Linux/macOS (it built its scratch path from the Windows-only `TEMP` variable, falling back to the current directory) by using `tmpdir()` and cleaning up every created directory in `afterAll`; this also fixes the same file's `OCX_DISPOSABLE_HOST_ROOT must be an absolute disposable-host path` failures on Linux, since `tmpdir()` is always absolute
- fix(ci): restore the five package scripts the Go-port merge dropped (`prepare:native-package`, `verify:native-package`, `verify:native-install`, `test:native-launcher`, `build:publish`) and put the native staging step back in `prepack`, so the Windows `npm-global` job stops failing with `Missing script: "verify:native-package"`; a new guard asserts every script a workflow invokes is defined in a package.json
- fix(launcher): restore `resolveBun`'s `required` parameter, which a merge removed while leaving the two guards that read it, so a launcher that cannot resolve the bundled runtime prints its own diagnostic instead of dying on `ReferenceError: required is not defined`
- fix(codex): stop deleting the operator's root `model_context_window` and `model_auto_compact_token_limit` on every injection; injection never wrote those keys, so it no longer removes them, and a new integration regression proves the operator's root model, both context keys, `[agents] max_concurrent_threads_per_session` and `[features] default_mode_request_user_input` all survive injection, re-injection and native restore
...
- fix(responses): repair decorated apply_patch envelopes without rewriting arbitrary exec input
- fix(grok): translate collision-free native edit/read/shell tools through request-local Codex sinks, including Windows PowerShell history parsing
- docs(responses): record that reasoning replay scope/memo from upstream PR2313 remains unported until this head has the required identity invariants
...
- fix(oauth): redact upstream `error_description` text before it can reach a thrown refresh/exchange error's message on the ChatGPT, xAI, and Kimi paths, which previously embedded it verbatim (unlike the Anthropic, GitHub Copilot, and Codex-account-store refresh paths); the two client-facing sinks that relay a non-terminal refresh failure now redact again as well, while the allowlisted OAuth error code that terminal-refresh classification depends on stays readable
- fix(codex): redact a Codex warmup request's upstream JSON error detail before it can reach the add-account 401 response, the login-status poll, and the GUI's account toast; the warmup request's Authorization header carries a live access token, so an upstream body that echoed request content back could previously surface it to the client
- fix(privacy): extend the privacy scanner's scanned-file allowlist to cover `.go`, `.mdx`, `.astro`, `.mts`, `.py`, and `.bat` files, which were previously skipped entirely regardless of content, and recognize Go's own test-only paths (`_test.go`, test/testdata directories) as trusted fixture locations the same way `tests/` already is for TypeScript
