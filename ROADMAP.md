...
- **The flags gap survives on the search bars that compile their own `RegExp`.** API keys,
  Appearance, Changelog, Grok, Language & voice, Logs, Notifications, Storage, Subagents and Usage
  each still build `new RegExp(query, "i")` directly, so a pattern built as case-sensitive arrives
  case-insensitive there. These are collection searches rather than the shared settings row; moving
  them onto `settingsMatcher` is what would close it.
