...
| `src/lib/bun-runtime.ts` | Bundled-Bun resolution: the shared Node-safe validator rejects missing, unreadable, and placeholder-sized binaries; `bundledBunPath()` and `durableBunPath()` prefer a valid `OPENCODEX_BUN_PATH`, with the durable choice baked into service/shim artifacts. |
| `src/lib/bun-start-supervisor.mjs` | Node-side child supervisor for the npm launcher. Forwards stderr live while retaining a 64 KiB attempt-local tail, retries only `start`/`ensure` after an abnormal exit carrying Bun's exact crash marker, and caps the retry at one. Spawn failures, ordinary nonzero exits, parent signals, warning-only output, and marker-free signal-style codes never retry. |
...
attempt-local stderr contains Bun's exact `oh no: Bun has crashed` marker. Every stderr byte is still
forwarded, the second result remains authoritative, and a second qualified crash prints the
