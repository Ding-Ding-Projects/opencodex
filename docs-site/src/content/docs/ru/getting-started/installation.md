...
opencodex устанавливает два эквивалентных имени команды: `ocx` и `opencodex`. В поддерживаемых npm-установках
обе запускают небольшой локальный HTTP-сервер на упакованном рантайме Go. Запросы к моделям идут к провайдеру,
...
Локальный маршрут по умолчанию сразу работает с существующим входом ChatGPT/Codex. API-ключ
OpenAI или любого другого провайдера для него не нужен.

## Предварительные требования

| Требование | Зачем |
| --- | --- |
| **[Node](https://nodejs.org) ≥ 18** | Небольшой загрузчик проверяет и запускает точный бинарный файл Go для macOS, Linux или Windows на amd64/x64 и arm64. Устанавливать Bun самостоятельно **не нужно**. |
...
| Вход ChatGPT/Codex | Встроенный маршрут `openai` по умолчанию пересылает сессию аккаунта, созданную `codex login` или приложением Codex. Это не API-ключ. |
| Аккаунт или API-ключ вышестоящего провайдера *(необязательно)* | Нужен только после явного добавления Anthropic, xAI, Kimi, Ollama Cloud, OpenRouter, другого хостингового провайдера или аутентифицируемой пользовательской конечной точки. |
...
:::note[Почему Bun всё ещё устанавливается?]
Зависимость Bun временно сохранена для старых updater-ов, однократного обновления прежнего Codex shim
и клиентов, которые явно используют Bun API пакета. Обычные команды на шести поддерживаемых целях
запускают Go, а не Bun. На неподдерживаемых платформах совместимый мост всё ещё может использовать Bun;
удаление зависимости отложено до завершения этих переходных путей.
:::
...
Сразу запустите локальный маршрут по умолчанию:

```bash
ocx start
```

Оставьте этот терминал открытым и выполните `ocx codex` в другом. Если Codex ещё не был авторизован,
один раз выполните `codex login`. Команда `ocx gui` открывает панель на актуальном порту и при
необходимости сначала запускает прокси.

:::note[Для локального запуска API-ключ не нужен]
**API-ключ вышестоящего провайдера** аутентифицирует только необязательный провайдер, который вы
добавили сами. **Ключ доступа OpenCodex** защищает `/v1/*` только при публикации прокси на
не-loopback адресе. Для маршрута `localhost` + вход ChatGPT/Codex не нужен ни один из них.
:::
...
Для работы над исходным кодом opencodex установите `bun` CLI локально и добавьте его в `PATH`:
...
Переходите к разделу [Быстрый старт](/ru/getting-started/quickstart/), чтобы запустить локальный
маршрут без ключа, или прочитайте [Как это работает](/ru/getting-started/how-it-works/),
