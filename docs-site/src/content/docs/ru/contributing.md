...
- `dev` — цель интеграции по умолчанию. Открывайте PR сюда, если он не относится к
  специализированной ветке ниже.
- `dev2-go` — параллельная линия интеграции для нативного порта на Go (`go/`, точка входа
  нативного рантайма и инструменты сборки Go-релизов). Принимает pull request'ы наравне с
  `dev`. Отправляйте сюда только работу, относящуюся к порту на Go; всё остальное — в `dev`.
  Автоматическая проверка целевой ветки допускает обе ветки и не различает их, поэтому
  границу определяет ревью: мейнтейнер может попросить сменить целевую ветку на `dev`.
...
Pull request'ы с портированием и ребейзом приветствуются. Перенос исправления между
линиями интеграции или ребейз устаревшей ветки на текущий head — это обычный вклад, а не
шум. Укажите исходные коммиты в описании.
