...
| `noProxy?` | `string \| string[]` | — | Ограниченный список хостов, адресов и host-портов для обхода. Настроенные значения не принимают URL, учётные данные, управляющие символы и wildcard; унаследованный стандартный `NO_PROXY`, включая `*` и суффиксы с точкой, сохраняется. Обход не разрешает частную сеть без `allowPrivateNetwork: true`. |
| `systemProxy?` | `"off" \| "static"` | `"off"` | Явное чтение статического WinINet-прокси только на Windows. PAC, WPAD, `AutoDetect` и неоднозначные значения `ProxyServer` по схемам отклоняются; явный `proxy` имеет приоритет, а неразрешимая `${ENV_VAR}` ссылка останавливает запуск. |
| `providerApiKeyVault?` | `"off" \| "windows"` | `"off"` | Необязательное хранение ключей провайдера в пользовательском DPAPI-хранилище. В конфигурации остаются только непрозрачные ссылки; полная резервная копия явно отклоняется, потому что ciphertext хранилища в неё не входит. |
...
| `stallTimeoutSec?` | `number` | `600` | Секунды без данных от вышестоящей стороны, после которых мост прерывает запрос и генерирует `response.incomplete`. Минимум 1. |
...
| `apiKeys?` | `OcxApiKey[]` | `[]` | Дополнительные сгенерированные учётные данные `ocx_…` для data plane на не-loopback-привязках. Они не являются ADMIN-данными. Управляются дашбордом; поля записей перечислены ниже. |
...
в не-loopback-адрес, такой как `0.0.0.0`, data plane и management API защищаются разными
учётными данными.
...
Без этой переменной или сгенерированного `apiKeys` прокси отказывается запускаться при привязке за
пределами loopback. Если вы устанавливаете фоновый сервис для доступа по LAN, экспортируйте ту же
переменную перед `ocx service install`, чтобы её получил launchd, systemd или Task Scheduler.
Data-plane-клиенты должны включать credential в каждый запрос через `x-opencodex-api-key`:
...
после запуска можно использовать вместо токена из окружения, но они не дают доступ к `/api/*`.

Для management API и удалённого дашборда нужен отдельный ADMIN token. OpenCodex создаёт его в
защищённом локальном secret file при запуске либо использует `OPENCODEX_ADMIN_AUTH_TOKEN`.
Удалённый дашборд запрашивает это ADMIN-значение и отклоняет data-plane key. Все credential
сравниваются за константное время (`timingSafeEqual`).

В Material 3 диалог **Подключиться к другому OpenCodex** принимает IPv4, IPv6 или DNS-имя и
фактический порт удалённого процесса. Он не выполняет probe, не помещает token в URL и не сохраняет
его. Если автоматический запуск выбрал fallback-порт, используйте live-значение из
`ocx host status` или `ocx status`, а не только `config.port`.

:::caution[Доступ по LAN]
Привязка к `0.0.0.0` открывает ваш прокси — и все настроенные учётные данные провайдеров —
локальной сети. HTTP не шифруется. Делайте это только в доверенных сетях, задавайте надёжные
data-plane и ADMIN credentials и предпочитайте SSH port forwarding на недоверенных маршрутах.
:::
