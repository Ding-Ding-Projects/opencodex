...
Запускает прокси-сервер (предпочтительный порт `10100`). Автоматический запуск без `--port`, если
предпочтительный порт занят, записывает другой свободный порт в runtime-состояние и направляет
Codex на живой listener, не меняя настройку предпочтительного порта. Явный `--port` является
жёсткой фиксацией: opencodex ждёт именно этот порт и завершается с ошибкой, не переходя на другой.
Передача после обновления и перезапуск из дашборда также сохраняют захваченный живой порт.
Межпроцессная блокировка и стабильная проверка identity health OpenCodex не дают одновременным
запускам создать второй daemon.

Перед восстановлением старого journal команды `start` и `ensure` проверяют identity существующего
исправного владельца прокси. Поэтому устаревший PID launcher не приводит к восстановлению состояния
Codex действующего прокси. Предупреждение о предыдущем сеансе сообщает о восстановлении и не является
классификатором сбоя Bun.

Внешний Node npm launcher контролирует только прямые вызовы `start` и `ensure`. Он передаёт stderr Bun
байт в байт, сохраняя для классификации только последние 64 KiB отдельно для каждой попытки. Повтор
выполняется ровно один раз и только после аварийного завершения, если этот хвост содержит точный маркер
`oh no: Bun has crashed`. Обычные ошибки не повторяются. `OPENCODEX_BUN_PATH` используется только для
доступного для чтения обычного файла размером не менее 1 000 000 байт (примерно 1 MiB); отклонённый путь
не печатается, а launcher возвращается к поставляемому Bun. Эта проверка не подтверждает identity
бинарного файла и не проверяет подпись издателя. Подробнее: [восстановление после сбоя Bun при запуске в
...
### `ocx codex [аргументы Codex...]`

Запускает Codex через opencodex. Если исправный прокси не запущен, команда поднимает его в фоне
на настроенном порту, обновляет конфигурацию провайдера Codex по фактическому порту и передаёт все
оставшиеся аргументы выбранной среде выполнения Codex без изменений.

```bash
ocx codex
ocx codex exec --skip-git-repo-check "Ответь READY"
```

### `ocx stop`

Останавливает работающий прокси и восстанавливает нативный Codex. Если установлен управляемый
фоновый сервис, `ocx stop` сначала подтверждает остановку менеджера, чтобы тот не породил прокси
снова. Затем команда проверяет identity-based завершение прокси и только после этого удаляет
runtime-состояние и маршрутизацию Codex/Grok. При неопределённости сервиса или прокси операция
закрывается безопасно и сохраняет принадлежащее OpenCodex состояние. То же действие доступно по
кнопке **Stop** веб-дашборда (`POST /api/stop`).
...
не делает.
Внешний launcher применяет к `ensure` описанное для `start` сочетание аварийного завершения и точного
маркера и делает не более одного повтора; обычный ненулевой результат не повторяется.
...
Без подкоманды перечисляет модели, статически заданные в настроенных провайдерах. `--provider`
фильтрует одного провайдера, а `--json` возвращает метаданные моделей. Действующий каталог читается
командой `ocx models live`.

Все операции над отдельными моделями, доступные в дашборде, есть и в CLI. `add`, `remove` и
`list-custom` работают с файлом конфигурации и применяются к запущенному прокси через синхронизацию
каталога; остальные обращаются к работающему management API, поэтому прокси должен быть запущен
(`ocx start` или установленная служба).

Подкоманды: `list`, `live`, `add`, `edit`, `remove`, `list-custom`, `enable`, `disable`,
`provider <name> <on|off>`, `selected <provider>`, `context`, `shadow`. Полный список флагов и
примеров — в английской версии справочника.

`--modalities` принимает только `text`, `image` и `audio`. Codex разбирает это поле как закрытое
перечисление и отклоняет **весь каталог**, если встречает любое другое значение (#759).
