...
`--port` なしの自動起動では設定ポートを優先値として扱います。使用中なら空きポートを
`runtime-port.json` に記録し、設定上の優先値を変更せず Codex を identity-verified な実際の
リスナーへ向けます。`ocx start --port 8080` は固定指定であり、そのポートを使えなければ別の
ポートへ移らず失敗します。更新ハンドオフとダッシュボード再起動も実行中ポートを固定します。
