...
パッケージ版ダッシュボードは Material 3 インターフェースだけを提供します。廃止済み UI、そのソース、
...
| **リモート接続** | IPv4、IPv6、ホスト名と実行中ポートを手入力して別の OpenCodex を開きます。管理 API は意図的に認証なしで開放されているため、ADMIN token を求められることはありません。 |
...
ナビゲーションレールまたはモバイルメニューから **別の OpenCodex に接続** を開きます。IPv4、IPv6、
...
確認、URL への token 追加、token 保存は行いません。移動先ダッシュボードも token を要求しません。
`/api/*` の管理ルートは意図的に認証なしで開放されているため ADMIN token は存在せず、data-plane
API key ともここでは無関係です。HTTP は暗号化されないため、直接接続は信頼できる LAN だけで使い、
それ以外では SSH tunnel を推奨します。

### エクスポートアーカイブ

安全な 7-Zip executable がある場合、暗号化なしの 7z は利用できますが、password 暗号化は無効です。
7-Zip は password を process arguments でしか受け取れず、同じマシンの別プロセスから見える可能性が
あるためです。OpenCodex は保護された password-input channel ができるまで暗号化を有効にしません。
