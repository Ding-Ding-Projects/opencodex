...
プロキシサーバーを起動します（優先ポート `10100`）。`--port` なしの自動起動では、優先ポートが使用中なら
別の空きポートをランタイム状態に記録し、設定上の優先値を変更せず Codex を実際のリスナーに同期します。
明示的な `--port` は固定指定です。その正確なポートを待ち、使用できなければ別ポートへ移らず失敗します。
...
実行中のプロキシを停止し、ネイティブ Codex を復元します。管理型バックグラウンドサービスがある場合は、
先にサービスマネージャーの停止を確認して再起動を防ぎます。その後プロキシの identity-based 終了を確認してから
ランタイム状態や Codex/Grok 設定を除去します。サービスまたはプロキシの状態が不明なら fail closed で所有状態を
保持します。ウェブダッシュボードの **Stop** ボタン（`POST /api/stop`）も同じ操作を行います。
