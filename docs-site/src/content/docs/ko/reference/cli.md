...
프록시 서버를 시작합니다(우선 포트 `10100`). `--port` 없는 자동 시작은 우선 포트가 사용 중이면
다른 빈 포트를 런타임 상태에 기록하고, 설정의 우선값을 바꾸지 않은 채 Codex를 실제 리스너에
동기화합니다. 명시적인 `--port`는 고정 지정입니다. 정확히 그 포트를 기다린 뒤 쓸 수 없으면 다른
포트로 옮기지 않고 실패합니다. 업데이트 인계와 대시보드 재시작도 캡처한 실행 포트를 고정합니다.
...
실행 중인 프록시를 중지하고 네이티브 Codex를 복원합니다. 관리형 백그라운드 서비스가 있으면 먼저
서비스 관리자 중지를 확인해 프록시 재생성을 막습니다. 그 다음 identity 기반으로 프록시 종료를
확인한 뒤에만 런타임 상태와 Codex/Grok 설정을 제거합니다. 서비스나 프록시 상태가 불확실하면 fail
closed로 소유 상태를 보존합니다. 웹 대시보드의 **Stop** 버튼(`POST /api/stop`)도 같은 작업을 수행합니다.
