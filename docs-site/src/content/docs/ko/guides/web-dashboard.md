...
| **원격 연결** | IPv4, IPv6 또는 호스트 이름과 실제 실행 포트를 직접 입력해 다른 OpenCodex를 엽니다. 관리 API는 의도적으로 인증 없이 열려 있으므로 ADMIN token을 요구받는 일은 없습니다. |
...
확인하거나 URL에 token을 추가하거나 token을 저장하지 않습니다. 대상 대시보드도 token을 요구하지
않습니다. `/api/*` 관리 라우트는 의도적으로 인증 없이 열려 있으므로 ADMIN token 자체가 없으며,
data-plane API key와도 무관합니다. HTTP는 암호화되지 않으므로 신뢰할 수 있는 LAN에서만 직접
연결하고, 다른 네트워크에서는 SSH tunnel을 사용하세요.
