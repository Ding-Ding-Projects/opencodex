...
`--port` 없는 자동 시작은 설정 포트를 우선값으로 취급합니다. 사용 중이면 빈 포트를
`runtime-port.json`에 기록하고, 설정 우선값을 바꾸지 않은 채 Codex를 identity-verified 실제
리스너로 보냅니다. `ocx start --port 8080`은 고정 지정이므로 해당 포트를 쓸 수 없으면 다른
포트로 옮기지 않고 실패합니다. 업데이트 인계와 대시보드 재시작도 실행 중 포트를 고정합니다.
