...
/**
 * Four glyphs stay hand-drawn, each for a reason that is not "we prefer ours".
 *
 * The three window controls are Windows title-bar chrome, not app iconography.
 * Windows draws minimise/maximise/restore as thin geometric marks (the Segoe
 * Fluent convention) and a filled Material glyph in that corner reads as a
 * foreign object on the OS it is running on. The GitHub mark is a third-party
 * brand logo and simply does not exist in Material Symbols — substituting a
 * generic glyph would misidentify the destination.
 */
const KEEP: Record<string, string> = {
  IconWinMinimize: `<svg {...SS(p)} strokeWidth={1.5}><path d="M6 12h12"/></svg>`,
  IconWinMaximize: `<svg {...SS(p)} strokeWidth={1.5}><rect x="6.5" y="6.5" width="11" height="11" rx="1"/></svg>`,
  IconWinRestore: `<svg {...SS(p)} strokeWidth={1.5}><rect x="5.5" y="8.5" width="10" height="10" rx="1"/><path d="M8.5 5.5h10v10"/></svg>`,
 ...
 *
 * Four icons are hand-drawn on purpose and say so where they are defined.
 */
...
/** Stroke geometry, for the four glyphs Material Symbols cannot supply. */
const SS = (props: P) => ({
