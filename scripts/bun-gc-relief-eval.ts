...
type ChildHandle = {
  ...
  nextGcReceipt(): Promise<{ type: "gc"; at: number; durationMs: number }>;
  rss(): number;
  stop(): Promise<void>;
};
...
async function startChild(upstreamUrl: string, dir: string): Promise<ChildHandle> {
  ...
  let gcWaiter: ((receipt: { type: "gc"; at: number; durationMs: number }) => void) | null = null;
  let gcRejecter: ((error: Error) => void) | null = null;
      ...
      for (const line of lines) {
        try {
          ...
          if (value.type === "gc" && gcWaiter && typeof value.at === "number" && typeof value.durationMs === "number") {
            const w = gcWaiter; gcWaiter = null; gcRejecter = null; w({ type: "gc", at: value.at, durationMs: value.durationMs });
          }
          if (value.type === "gc-error" && gcRejecter) {
            const reject = gcRejecter; gcWaiter = null; gcRejecter = null;
            reject(new Error("child GC evaluation failed"));
          }
        } catch { /* startup noise */ }
      }
    }
  ...
  return {
    ...
    nextGcReceipt: () => new Promise((resolve, reject) => {
      ...
      gcRejecter = reject;
      setTimeout(() => {
        if (gcWaiter) { gcWaiter = null; gcRejecter = null; reject(new Error("gc receipt timeout")); }
      }, 10_000);
    }),
    rss: () => {
...
async function runCell(kind: CellKind, arm: Arm, runIndex: number): Promise<CellResult> {
  ...
  try {
    child = await startChild(fixture.url, dir);
  } catch {
    await fixture.stop();
    return { kind, arm, runIndex, ok: false, error: "child_start_failed" };
  }
  ...
  try {
    ...
    return {
      ...
      runIndex,
      ok: false,
      error: error instanceof Error ? error.message.slice(0, 240) : "cell_failed",
    };
  } finally {
    await child.stop();
    await fixture.stop();
...
// Deliberately serial: each cell owns a fresh child process and its allocator baseline.
// Parallel cells contaminate RSS and make the A/B result uninterpretable.
...
const errors = results.filter(result => result.ok !== true);
const report = {
  schemaVersion: 1,
  source: SOURCE,
  smoke: SMOKE,
  ...
  freshChildPerCell: true,
  verdict: errors.length > 0 ? "error" : "measurement_only",
  errors,
  results,
};
