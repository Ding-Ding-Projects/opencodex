 ...
 * serial `for` loop. Each spawn is independent of every other, so at roughly 5,000 tracked code
 * files and several milliseconds of process-launch overhead apiece, that loop alone cost tens of
 * seconds on a fast machine and minutes on a slow one. It also ran at module import time in
 * `tests/count-lines-attribution.test.ts`, outside any per-test timeout, which is how that file
 * came to stall the root suite. `line-attribution.ts` had already solved the identical shape of
 * problem for its own per-file `git blame` calls by routing them through
 * {@link mapWithConcurrency}; this does the same for `git show`, through the exact same helper
 * (see `./concurrency`).
 */
 ...
 * over `maxBuffer`), exactly as the previous synchronous version treated every one of those:
 * the file becomes "unreadable" rather than the whole count failing.
 */
