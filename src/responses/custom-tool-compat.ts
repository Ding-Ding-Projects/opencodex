...
export function restoreRoutedCustomCallsInJson(
  ...
  if (!restored.changed) return text;
  // A converted function_call_output has no name of its own. Pair it with the
  // request-local call id that was just restored so unauthorized outputs remain
  // untouched and the custom-tool lifecycle stays complete.
  const restoredCallIds = new Set<string>();
  const collect = (value: unknown): void => {
    if (Array.isArray(value)) { for (const entry of value) collect(entry); return; }
    if (!isPlainObject(value)) return;
    if (value.type === "custom_tool_call" && typeof value.call_id === "string") restoredCallIds.add(value.call_id);
    for (const entry of Object.values(value)) collect(entry);
  };
  collect(restored.value);
  const convertOutputs = (value: unknown): unknown => {
    if (Array.isArray(value)) return value.map(convertOutputs);
    if (!isPlainObject(value)) return value;
    const next: Record<string, unknown> = {};
    let changedOutput = false;
    for (const [key, entry] of Object.entries(value)) {
      const converted = convertOutputs(entry);
      next[key] = converted;
      changedOutput ||= converted !== entry;
    }
    if (value.type === "function_call_output" && typeof value.call_id === "string" && restoredCallIds.has(value.call_id)) {
      next.type = "custom_tool_call_output";
      changedOutput = true;
    }
    return changedOutput ? next : value;
  };
  return JSON.stringify(convertOutputs(restored.value));
}
