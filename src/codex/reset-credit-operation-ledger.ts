...
function initializeConfigGeneration(database: Database): void {
  database.exec("CREATE TABLE IF NOT EXISTS config_generation (singleton INTEGER PRIMARY KEY CHECK (singleton = 1), value INTEGER NOT NULL CHECK (value >= 0)); INSERT OR IGNORE INTO config_generation (singleton, value) VALUES (1, 0)");
}
