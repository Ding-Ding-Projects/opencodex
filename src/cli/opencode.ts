...
async function ensureProxyForOpencode(config: OcxConfig): Promise<LiveProxy | null> {
  ...
  // If another caller won the startup lock, its healthy proxy is safe to adopt after
  // the exact-child wait expires; liveness still performs the identity check.
  return started ?? await waitForProxyIdentity({ intervalMs: 250 });
}
