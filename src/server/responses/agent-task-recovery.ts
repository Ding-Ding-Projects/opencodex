...
export type AgentTaskRecoveryState = "off" | "on" | "ineligible";

/** Public, non-secret compatibility state for management/UI surfaces. */
export function agentTaskRecoveryState(config: OcxConfig, hasRoutedV2Child: boolean): {
  state: AgentTaskRecoveryState;
  enabled: boolean;
  eligible: boolean;
  reason: string;
} {
  const enabled = config.agentTaskRecovery?.enabled === true;
  if (!hasRoutedV2Child) return { state: "ineligible", enabled, eligible: false, reason: "No routed V2 child is selected." };
  if (!enabled) return { state: "off", enabled: false, eligible: true, reason: "Experimental recovery is disabled; plaintext V2 or a native child remains available." };
  return { state: "on", enabled: true, eligible: true, reason: "Experimental recovery may normalize unreadable encrypted tasks with bounded cache, quota, latency, and fidelity tradeoffs." };
}
