 ...
 *
 * Endpoints (behind management auth except for the token-bound pairing claim):
...
export async function handleHostRoutes(ctx: ManagementContext): Promise<Response | null> {
  ...
  if (url.pathname === "/api/host" && req.method === "PUT") {
    ...
    // A user-supplied key is still a live credential. Refuse it before any
    // branch can validate, append, persist, or echo the supplied material; the
    // library helper carries the same backstop for future callers.
    if (typeof body.customKeyValue === "string" && debugSandboxEnabled()) {
      announceDebugSandboxOnce();
      return noStore(jsonResponse({
        error: `${DEBUG_SANDBOX_ENV} is set: custom data-plane keys are disabled in the debug sandbox.`,
      }, 409, req, config));
    }
  ...
  if (isUnauthenticatedPairingClaim(req.method, url.pathname)) {
    ...
    try { body = JSON.parse(raw) as typeof body; } catch {
      return noStore(jsonResponse({ error: "Invalid JSON" }, 400, req, config));
    }
