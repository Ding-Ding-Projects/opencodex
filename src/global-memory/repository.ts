...
export async function resolveGlobalMemoryRepository(
  ...
  if (platform !== "win32") {
    throw new GlobalMemoryRepositoryError("Global memory synchronization is supported on Windows only.");
  }
  ...
  const synchronizerPath = await ensureRegularFile(repositoryPath, join(repositoryPath, "scripts", "sync-agent-memory.ps1"), "Windows canonical synchronizer");
  return {
    repositoryPath,
    origin,
    payloadPath,
