...
function makeRepository(options: { origin?: string; payload?: boolean; skill?: boolean; script?: boolean } = {}): string {
  ...
  if (options.script !== false) {
    writeFileSync(join(repository, "scripts", "sync-agent-memory.sh"), "#!/usr/bin/env bash\n");
...
function repositoryFor(platform: "win32" | "posix"): GlobalMemoryRepository {
  ...
  return {
    ...
    synchronizerPath: join(repository, "scripts", platform === "win32" ? "sync-agent-memory.ps1" : "sync-agent-memory.sh"),
    platform,
  };
}
...
describe("global-memory synchronizer process boundary", () => {
  test("constructs Windows and POSIX commands without a shell", () => {
    const win = repositoryFor("win32");
    ...
    const posix = repositoryFor("posix");
    expect(buildSynchronizerCommand(posix, { action: "status", targets: ["all"] })).toEqual({ command: "bash", args: [posix.synchronizerPath, "status", "--target", "all"] });
  });
