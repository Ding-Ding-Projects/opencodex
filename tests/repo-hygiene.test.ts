...
describe("repository hygiene", () => {
  ...
  test("no source turns a file URL into a path with .pathname", async () => {
    ...
    for (const path of candidates) {
      // The index can name a path that is not on disk: mid-merge, mid-rename, or with a
      // generated file's hashed name already replaced. That is a staging state, not a source
      // file with a defect in it, and reading it blindly turns this guard red for a reason
