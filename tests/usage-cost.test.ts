...
describe("product and condition isolation", () => {
  ...
  test("prices supported OpenAI direct IDs and preserves unavailable cache writes", () => {
    ...
    // Short-context rates are unchanged by the band work: same figures as before.
    const short = { usage, promptInputTokens: 1_000 };
    expect(requestCost("openai-apikey", "gpt-5.6-sol", short)?.cost.total).toBeCloseTo(7.85, 9);
    expect(requestCost("openai-apikey", "gpt-5.6-terra", short)?.cost.total).toBeCloseTo(3.14, 9);
    expect(requestCost("openai-apikey", "gpt-5.6-luna", short)?.cost.total).toBeCloseTo(0.314, 9);
    expect(requestCost("openai-apikey", "gpt-5.5", short)?.cost.total).toBeCloseTo(7.85, 9);
