...
import { sevenZipArgs } from "../src/lib/export-archive";
...
test("the export routes stay behind management auth and work with the admin credential", async () => {
  const server = startServer(0);
  try {
    const unauthenticated = await globalThis.fetch(new URL("/api/export/capabilities", server.url));
    expect(unauthenticated.status).toBe(401);
    const authenticated = await managementFetch(new URL("/api/export/capabilities", server.url));
...
test("the export surface cannot open an arbitrary caller-supplied local path", async () => {
  // Opening a server-owned export is requested as part of POST /api/export.
  // A general path opener would let any administrator-controlled browser ask
  // the host to launch an arbitrary local path, which is broader than the GUI
  // needs and creates an unnecessary filesystem/process boundary.
  const server = startServer(0);
  try {
    const res = await managementFetch(new URL("/api/export/open", server.url), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
test("encrypted 7z requests are rejected before a password can reach process argv", async () => {
  expect(() => sevenZipArgs("out.7z", { password: "one-use-secret" }))
    .toThrow("exposes passwords in process arguments");
  expect(() => sevenZipArgs("out.7z", { encryptHeaders: true }))
    .toThrow("encrypted 7z export is disabled");
  ...
  try {
    const res = await managementFetch(new URL("/api/export", server.url), {
      ...
      body: JSON.stringify({
        ...
        format: "json",
        archive: "7z",
        sevenZip: { password: "one-use-secret", encryptHeaders: true },
      }),
    });
    expect(res.status).toBe(400);
