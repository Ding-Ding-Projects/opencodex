    ...
    providers: {
      openai: {
        ...
        mcpServers: {
          files: {
            command: "npx",
            args: ["-y", "server", "--api-key", "mcp-SECRET-VALUE"],
            env: { TOKEN: "mcp-SECRET-VALUE" },
          },
          remote: {
            url: "https://user:SECRET-VALUE@mcp.example.test/rpc/SECRET-VALUE?token=SECRET-VALUE#SECRET-VALUE",
...
describe("the export registry", () => {
  ...
  test("API-key rows contain no secret-derived substring, even for minimum-length custom keys", () => {
    const customKey = "abcdefghijkl"; // the accepted twelve-character floor
    const config = configWithSecrets();
    config.apiKeys = [{
      id: "minimum",
      name: "custom",
    ...
    const rows = DATASETS.find(dataset => dataset.id === "api-keys")!.rows(config);
    expect(rows).toEqual([{
      id: "minimum",
      name: "custom",
      createdAt: "2026-01-02T00:00:00Z",
    }]);
    expect(textOf(rows)).not.toContain(customKey);
  });

  test("MCP servers export only structural metadata, never raw credential carriers", () => {
    ...
    expect(textOf(rows)).not.toContain("SECRET-VALUE");
    for (const forbidden of ["command", "args", "cwd", "url", "envKeys", "headerKeys"]) {
      expect(rows.every(row => !Object.hasOwn(row, forbidden))).toBe(true);
    }
  });
