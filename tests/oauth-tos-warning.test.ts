...
describe("oauth ToS warning UI seam", () => {
  test("Providers and AddProvider gate OAuth login behind the warning modal", async () => {
    ...
    // The warning is a real modal <dialog>, but the M3 component delegates its
    // native lifecycle to the shared Dialog rather than duplicating it here.
    expect(warn).toContain("<Dialog");
    expect(dialog).toContain("showModal()");
    // Both native dismissal and the visible Cancel action reach the caller.
