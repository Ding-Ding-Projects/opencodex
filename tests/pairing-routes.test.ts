 ...
 * management plane remains admin-authenticated; only `POST /api/host/pair/claim`
 * has its own 400/429/413 contract and never treats a malformed scan as an
...
describe("the unauthenticated claim", () => {
  test("is the only reachable management action without a credential", async () => {
    saveConfig(baseConfig());
    const server = startServer(0);
    try {
      // Sibling management routes remain protected; an invalid claim is refused
      // by the pairing handler itself, not by the admin-auth gate.
  ...
  test("never answers 401, whatever is wrong with the request", async () => {
    ...
    try {
      ...
      const invalidJson = await globalThis.fetch(new URL("/api/host/pair/claim", server.url), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: "{not json",
      });
      statuses.push(invalidJson.status);
  ...
  test("only POST is a claim handler — the path alone does not create another route", async () => {
    ...
    const server = startServer(0);
    try {
      const unauthenticated = await globalThis.fetch(new URL("/api/host/pair/claim", server.url), { method: "GET" });
      expect(unauthenticated.status).toBe(401);
      const authenticated = await managementFetch(new URL("/api/host/pair/claim", server.url), { method: "GET" });
...
describe("minting and cancelling", () => {
  test("both require management authentication", async () => {
    saveConfig(baseConfig());
    const server = startServer(0);
    try {
      const rawMint = await globalThis.fetch(new URL("/api/host/pair", server.url), { method: "POST" });
      expect(rawMint.status).toBe(401);
      const rawCancel = await globalThis.fetch(new URL("/api/host/pair", server.url), { method: "DELETE" });
      expect(rawCancel.status).toBe(401);
      expect((await managementFetch(new URL("/api/host/pair", server.url), { method: "POST" })).status).toBe(200);
      expect((await managementFetch(new URL("/api/host/pair", server.url), { method: "DELETE" })).status).toBe(200);
    } finally {
      await server.stop(true);
    }
  });
