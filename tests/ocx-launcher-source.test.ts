...
describe("ocx.mjs npm launcher (source invariants)", () => {
  ...
  test("valid Bun overrides are selected before the bundled runtime", () => {
    ...
    // The invariant is the ORDER inside resolveBun, not its signature: anchor on the declaration
    // rather than an exact parameter list, so restoring the `required` parameter the soft-failure
    // callers pass does not read as an ordering regression.
