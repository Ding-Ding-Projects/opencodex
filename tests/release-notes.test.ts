...
describe("rewriteTakeoverCredits", () => {
  test("credits original author and keeps landing PR link", async () => {
    const body = [
      "## What's Changed",
      "### New Features",
      "* feat(images): Grok image bridge (maintainer takeover of #424) by @Wibias in https://github.com/Ding-Ding-Projects/opencodex/pull/577",
      "* feat(other): normal change by @Alice in https://github.com/Ding-Ding-Projects/opencodex/pull/100",
    ].join("\n");
    ...
    expect(rewritten).toContain(
      "* feat(images): Grok image bridge (maintainer takeover of #424) by @tizerluo (takeover by @Wibias) in https://github.com/Ding-Ding-Projects/opencodex/pull/577",
    );
    expect(rewritten).toContain(
      "* feat(other): normal change by @Alice in https://github.com/Ding-Ding-Projects/opencodex/pull/100",
    );
  ...
  test("falls back to landing lookup when title says takeover without #N", async () => {
    const line =
      "* feat x (takeover) by @Wibias in https://github.com/Ding-Ding-Projects/opencodex/pull/577";
    const rewritten = await rewriteTakeoverCredits(
      line,
      async (pr) => {
    ...
    expect(rewritten).toContain(
      "* feat x (takeover) by @tizerluo (takeover by @Wibias) in https://github.com/Ding-Ding-Projects/opencodex/pull/577",
    );
  });
  ...
  test("leaves line unchanged when original author matches landing author", async () => {
    const line =
      "* feat x (takeover #9) by @Wibias in https://github.com/Ding-Ding-Projects/opencodex/pull/10";
    const rewritten = await rewriteTakeoverCredits(
      line,
      async () => ({
