...
describe("Grok inject model catalog", () => {
  test("applies native OpenAI context limits to Grok rows", () => {
    ...
    expect(defaults.find(model => model.id === NATIVE_SOL)?.contextWindow).toBe(372_000);

    expect(defaults.find(model => model.id === NATIVE_SOL)?.reasoningEfforts)
      .toEqual(nativeReasoningEfforts(NATIVE_SOL));
  });
});
