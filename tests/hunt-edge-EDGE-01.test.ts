...
describe("EDGE-01: management-api content-length size gate", () => {
  ...
  test("a non-numeric content-length header is rejected as malformed, not silently allowed", async () => {
    ...
    const response = await handleManagementAPI(req, new URL(req.url), minimalConfig(), {});
    // Fix applied in this repository: the gate now treats a header that is present but not a
    // finite non-negative integer as malformed and rejects it with 400 before any handler runs,
    // distinct from the 413 above which means "the header parsed cleanly and the body really is
    // too big". A non-numeric header cannot prove the body exceeds 2 MiB, only that the request
    // itself is not well formed, so 400 is the more accurate status here; either 400 or 413 would
    // have closed this hole, and 400 is the one this fix chose. Before the fix,
    // `Number.isFinite(NaN)` silently short-circuited the whole check to "allow", so this
    // unmatched route fell through to the function's final `return null` instead of ever being
    // rejected.
