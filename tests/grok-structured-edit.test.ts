...
describe("Grok structured edit tools", () => {
  ...
  test("rewrites read_file into exec_command cat", async () => {
    ...
    expect((events[1] as { arguments: string }).arguments).toContain(process.platform === "win32" ? "EncodedCommand" : "cat --");
    expect((events[1] as { arguments: string }).arguments).toContain(process.platform === "win32" ? "EncodedCommand" : "README.md");
  });
  ...
  test("rewrites ranged read_file into BSD-safe sed", async () => {
    ...
    expect(args).toContain(process.platform === "win32" ? "EncodedCommand" : "sed -n '160,$p' 'utils/database.py'");
  ...
  test("converts live Codex list_dir/read_file calls into exec", async () => {
    ...
    expect(args[0]?.arguments).toContain(process.platform === "win32" ? "EncodedCommand" : "ls -la --");
    expect(args[1]?.arguments).toContain(process.platform === "win32" ? "EncodedCommand" : "cat --");
    expect(args[1]?.arguments).toContain(process.platform === "win32" ? "EncodedCommand" : "README.md");
  });
