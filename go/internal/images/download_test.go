...
// helper in internal/cli/config_parity_test.go and internal/grok/inject_test.go:
// os.Chmod cannot set real POSIX permission bits on Windows (a writable file
// always reports back as 0666/-rw-rw-rw- no matter what mode was requested),
// so a Windows pass checks the real thing HardenSecretPath actually did
// instead -- that icacls no longer finds any of the three broad SIDs it
// strips (Everyone, Authenticated Users, BUILTIN\Users) on the path.
func assertOwnerOnlyPermissions(t *testing.T, path string) {
	...
	for _, sid := range []string{"*S-1-1-0", "*S-1-5-11", "*S-1-5-32-545"} {
		...
		if strings.Contains(string(output), path) {
			t.Fatalf("artifact left broad SID %s grantable on %s: %s", sid, path, output)
		}
	}
}
