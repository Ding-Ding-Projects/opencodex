...
ocx start                      # автозапуск; при занятом предпочтительном порте выбирается свободный
ocx start --port 10100         # явная фиксация; перехода на другой порт нет
...
shim включён по умолчанию и отключается в GUI-панели управления. Автозапуск без `--port` считает
настроенный порт предпочтительным; если он занят, другой свободный порт записывается в runtime state,
а Codex направляется на фактический listener. Явный `ocx start --port`, передача после обновления и
перезапуск из дашборда фиксируют тот же порт и завершаются с ошибкой, не переходя на другой.
...
По умолчанию opencodex привязывается к `127.0.0.1` (loopback). Если вы задаёте
`"hostname": "0.0.0.0"`, плоскость данных (`/v1/responses`, `/v1/images/generations` и
`/v1/images/edits`) требует отдельный bearer credential:
...
Дашборд и `/api/*` используют отдельный ADMIN token. Для подключения к другому OpenCodex введите
его IP/hostname и фактический порт из `ocx host status` (или `ocx status`), затем ADMIN token того
прокси в prompt целевого дашборда.
Data-plane key не принимается как ADMIN credential, а диалог не помещает token в URL и не сохраняет
его. HTTP не шифруется: прямое подключение допустимо только в доверенной LAN; иначе используйте SSH
tunnel. Tokens сравниваются за постоянное время для защиты от атак по времени.
