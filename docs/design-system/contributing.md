...
ESLint는 더 이상 게이트가 아니다. 어떤 GitHub Actions 워크플로도 ESLint를 실행하지
않고, pre-push 훅도 실행하지 않으며, lint 결과로 빌드나 릴리스가 막히지 않는다.
`bun run lint`(저장소 루트에서는 `bun run lint:gui`)는 그대로 설치되어 있으니
위 체크리스트대로 직접 실행하고 나온 오류는 고친다.

대가는 분명히 적어 둔다. ESLint가 지적했을 커밋에서도 릴리스가 나갈 수 있고, 그
사실을 처음 알아차리는 것은 빨간 체크가 아니라 코드를 읽는 사람이다. 파이프라인이
초록이라고 해서 "lint를 통과했다"고 말하면 안 된다. 실행 자체를 하지 않았다.
