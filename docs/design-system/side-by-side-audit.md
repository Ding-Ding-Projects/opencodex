...
swept against the shared component library instead; that record follows.
...
Four of the five Part-one defects were closed on 2026-08-15, each with a test watched red before
it was trusted. The fifth, the missing Providers **Test connection** control, was closed in the
same pass by a separate lane — the backend probe had existed all along at
`POST /api/providers/test`, already backing `ocx provider test`, with four strings translated into
all seven locales and no caller anywhere. Only the button was missing.
